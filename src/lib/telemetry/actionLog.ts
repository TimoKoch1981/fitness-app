...
export type ErrorCode =
  ...
  | 'agent_claim_no_block'      // Agent claimed to have saved sth but no parseable block found (silent halluzination)
  | 'date_clamped'              // clampDate silently corrected a bad date
  | 'provider_error'            // AIProvider threw (timeout, 5xx, etc.)
  | 'quota_exceeded'            // Per-user AI cost/token quota exceeded
  | 'unknown';
...
export interface LogActionEventParams {
  ...
  // ── Phase 2 / KI3: LLM-Cost-Tracking ──────────────────────────────────
  /** Prompt tokens consumed by the LLM call. */
  promptTokens?: number;
  /** Completion tokens generated by the LLM. */
  completionTokens?: number;
  /** LLM cost in 1/100 cents (integer for precision). e.g. 250 = 2.50 cents. */
  costCentsX100?: number;
  /** LLM model id ('gpt-4o-mini', 'claude-sonnet-4-6', etc.) */
  llmModel?: string;
}
...
export async function logActionEvent(params: LogActionEventParams): Promise<void> {
  try {
    ...
    const row = {
      ...
      cost_cents_x100: params.costCentsX100 ?? null,

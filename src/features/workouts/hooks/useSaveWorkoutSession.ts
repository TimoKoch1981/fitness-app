  ...
  return useMutation({
    ...
    mutationFn: withTelemetry('save_workout_session', 'ui', async (input: SaveSessionInput) => {
      ...
      const workoutPayload = {
        user_id: userId,
        date: today(),
        name: session.planDayName || 'Freies Training',
        type: 'strength' as const,
        duration_minutes: durationMinutes,
        calories_burned: caloriesBurned,
        exercises: legacyExercises,
        // Coerce empty strings to null — Free-Workout mode has no plan,
        // and Postgres rejects "" for uuid columns (22P02).
        plan_id: session.planId || null,
        plan_day_id: session.planDayId || null,
        plan_day_number: session.planDayNumber ?? null,

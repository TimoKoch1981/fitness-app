...
export function CapabilitiesSheet({ open, onClose, onSendMessage }: CapabilitiesSheetProps) {
  ...
  const capabilities: Capability[] = [
    { icon: <Utensils className="h-4 w-4 text-theme-success" />, labelKey: 'capNutrition', example: isDE ? '200g Hähnchen mit Reis und Salat' : '200g chicken with rice and salad' },
    ...
    { icon: <Activity className="h-4 w-4 text-theme-primary" />, labelKey: 'capBody', example: isDE ? 'Wie hat sich mein Gewicht entwickelt?' : 'How has my weight progressed?' },
        ...
        <div className="px-5 pb-6 space-y-2">
          {capabilities.map((cap) => (
            <button
              ...
              className="w-full flex items-start gap-3 p-3 bg-gray-50 rounded-xl hover:bg-theme-surface-2 transition-colors text-left group"
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-400 group-hover:text-theme-primary mt-0.5 truncate">

...
| # | Bug | Prio | Analyse |
|---|-----|------|---------|
| ~~B22~~ | ~~Multi-Day-Training: nur Tag 1 hat Start-Button~~ | ~~P0~~ | **GEFIXT 2026-05-18 (commit `1c5f68b`)** — DayCard rendert jetzt fuer Tage ohne Uebungen einen "+ Uebungen hinzufuegen"-Button (oeffnet PlanEditorDialog). Tage mit Uebungen zeigen wie gewohnt Start. **Live-verifiziert** auf fudda.de: Plan-Wizard-Fix-Test mit gemischtem Befuellungs-Stand zeigt korrektes Verhalten pro Tag. |
| ~~B23~~ | ~~Gewichte im Workout fehlen / Eingabe defekt / Last-Weight-Memory weg~~ | ~~P0~~ | **GEFIXT 2026-05-18 (commit `d4a9122`)** — `buildExercisesFromPlan` bekam Final-Fallback `?? 0` in der Strength-Priority-Chain (`ActiveWorkoutContext.tsx:156`). Damit ist `target_weight_kg` nie `undefined` → `noWeight`-Check triggert nicht → Spalte/Input sichtbar. **Live-verifiziert** auf fudda.de: Bankdrücken-Plan zeigt 30 kg im Input. Followup FU-3: `noWeight` sollte langfristig auf Catalog-Equipment basieren, nicht auf Symptom. |
| ~~B24~~ | ~~Uebungs-Videos defekt oder Button verschwunden~~ | ~~P0~~ | **GEFIXT 2026-05-18 (commit `505a6a1`)** — Diagnose: Catalog-Daten sind komplett (alle 183 Eintraege haben `video_url_de/en`). Echtes Problem: User-Plan-Namen weichen vom Catalog ab (`Bizepscurls` vs `Bizeps-Curls`, `Bankdrücken (Flachbank)` vs `Bankdrücken`, ASCII-Varianten). `findExerciseInCatalog` bekam `normalizeName()`-Pass (lowercase + Umlaut→ASCII + alle nicht-alnum-Zeichen weg). **Live-verifiziert** auf fudda.de: Video-Button (rotes Camera-Icon) neben Uebung. |
| ~~B25~~ | ~~Active-Workout-Theme: blaue Schrift auf schwarzem Grund schwer lesbar~~ | ~~P0~~ | **GEFIXT 2026-05-18 (commit `512e731`)** — Root-Cause: `bg-gray-800` Timer-Panels in `ActiveWorkoutPage.tsx:482+528` hatten `text-theme-primary` (Studio-Indigo `#3D4FB8`), Kontrast ~1.8:1 auf gray-800. Frueher (v14.27) war's `text-teal-400`, Kontrast ~6.8:1 — wurde von v14.28 Stufe 4 Color-Sweep mit-migriert. Fix: neuer Token `--theme-accent-on-dark` (Studio: Teal-400, Console: Acid-Lime), beides WCAG AAA. Followup FU-1: Tracker-Hardcodes `text-blue-*`/`text-indigo-*` bleiben in `docs/regression-sprint-followups.md` fuer naechsten Look-&-Feel-Sprint. |
...
## UX-Polish-Sprint (komplett abgeschlossen 2026-05-16)

- **Tranche 1 deployed:** UX2/9/10/13/15/8 (RPE-Picker, e1RM-Spalte,
  Plate-Calculator, Assay-Hinweis Hormone, Sparklines, Touch-44 Utility)
- **Tranche 2 deployed:** UX4/6/11/12 (Wegovy/Substanz-Reminder, JargonTerm-Glossar
  mit 15 Begriffen, Sets-pro-Muskelgruppe-Chart, Peak-Week-Modus mit
  Tag-fuer-Tag-Guidance)

Alle 10 UX-Items aus dem Persona-Review-Backlog gebaut, getestet und live.
